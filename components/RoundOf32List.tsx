...
// components/RoundOf32List.tsx
//
// Hiển thị danh sách (dạng bảng) các trận đấu Vòng 32 đội — World Cup 2026.
...
// Tự cập nhật realtime khi Edge Function ghi tỷ số mới.
...
async function fetchRoundOf32(): Promise<MatchRow[]> {
  ...
  if (error) {
    console.error("Không tải được danh sách vòng 32 đội:", error.message);
    return [];
  }
...
export default function RoundOf32List({ initialMatches }: { initialMatches: MatchRow[] }) {
  const [matches, setMatches] = useState<MatchRow[]>(initialMatches);

  useEffect(() => {
    const channel = supabase
      .channel("round_of_32_changes")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "knockout_matches" },
        async () => {
          setMatches(await fetchRoundOf32());
        },
      )
      .subscribe();
  ...
  if (sorted.length === 0) {
    return (
      <p className="text-slate-400">
        Chưa có dữ liệu trận đấu Vòng 32 đội. Hãy kiểm tra lại bảng{" "}
