...
import RoundOf32List from "@/components/RoundOf32List";
  ...
  return (
    <main className="min-h-screen bg-slate-900 p-8 text-white">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold">⚽ Vòng 32 đội — World Cup 2026</h1>
        <Link href="/bracket" className="text-sm text-emerald-400 hover:underline">
          Xem dạng Bracket →
        </Link>
      </div>
      ...
      {error ? (
        ...
        <RoundOf32List initialMatches={(matches ?? []) as unknown as MatchRow[]} />
      )}
    </main>
  );
}
